  ...
  for (const id of AGENT_IDS) {
    ...
    if (api) {
      if (api.status === 'active') {
        visualStatus = 'active'
        confidence = Math.max(20, Math.min(98, Math.round(100 - (api.last_heartbeat_age_s ?? 90) * 0.5)))
      } else if (api.status === 'idle' || api.status === 'stale') {
        visualStatus = 'idle'
        confidence = Math.max(5, Math.min(50, Math.round(50 - (api.last_heartbeat_age_s ?? 180) * 0.15)))
      }
    }
...
export default function AgentOrchestrationPage() {
  ...
  // Sync real agent statuses into visual state
  useEffect(() => {
    if (agentsData?.agents && !isRunning) {
      ...
      const updatedSteps = TIMELINE_STEPS.map((s) => {
        const api = apiAgents[s.agentId]
        const stepStatus = !api
          ? 'pending' as const
          : api.status === 'active'
            ? 'active' as const
            : api.status === 'idle' || api.status === 'stale'
              ? 'completed' as const
              : 'pending' as const
        return {
          ...s,
          status: stepStatus,
          agentName: api ? api.name : '',
        }
      })
    ...
    setSteps(
      agentsData?.agents
        ? TIMELINE_STEPS.map((s) => {
            const api = agentsData.agents[s.agentId]
            return {
              ...s,
              status: api?.status === 'active' ? 'active' as const : api?.status === 'idle' ? 'completed' as const : 'pending' as const,
              agentName: api?.name ?? '',
            }
          })
        : TIMELINE_STEPS
  ...
  const startDemo = useCallback(() => {
    ...
    const DEMO_EVENTS: { id: string; label: string; delay: number; activate: AgentType[]; complete: AgentType[]; edges: { active: string[]; complete: string[] }; message?: Omit<AgentMessage, 'id' | 'timestamp'> }[] = [
      { id: 'intake', label: 'Intake Complete', delay: 2000, activate: ['intake'], complete: [], edges: { active: [], complete: [] }, message: { from: 'Intake Agent', content: 'Referral received — processing child intake for emergency placement', type: 'info' } },
      { id: 'intake-done', label: 'Profile Loaded', delay: 4000, activate: [], complete: ['intake'], edges: { active: ['e-intake-planner'], complete: [] }, message: { from: 'Intake Agent', content: 'Child profile loaded — Age: 8, Priority: High', type: 'success' } },
      { id: 'plan', label: 'Planning', delay: 3000, activate: ['planner'], complete: [], edges: { active: [], complete: [] }, message: { from: 'Planner Agent', content: 'Created execution plan — spawning parallel assessment agents', type: 'info' } },
      { id: 'plan-done', label: 'Plan Finalized', delay: 4000, activate: [], complete: ['planner'], edges: { active: ['e-planner-risk', 'e-planner-matching', 'e-planner-fairness'], complete: ['e-intake-planner'] }, message: { from: 'Planner Agent', content: 'Risk, Matching, and Fairness agents activated', type: 'success' } },
      { id: 'risk', label: 'Risk Scoring', delay: 3000, activate: ['risk', 'matching', 'fairness'], complete: [], edges: { active: [], complete: [] }, message: { from: 'Risk Agent', content: 'Risk assessment initialized — analyzing case history', type: 'info' } },
      { id: 'risk-done', label: 'Risk Score: 45', delay: 5000, activate: [], complete: ['risk'], edges: { active: [], complete: ['e-planner-risk'] }, message: { from: 'Risk Agent', content: 'Risk Score = 45 (Low Risk) — No safety concerns', type: 'success' } },
      { id: 'match-done', label: 'Match Found', delay: 3000, activate: [], complete: ['matching'], edges: { active: [], complete: ['e-planner-matching'] }, message: { from: 'Matching Agent', content: 'Found 3 candidate families — Johnson Family ranked #1', type: 'success' } },
      { id: 'fair-done', label: 'Fairness: Pass', delay: 3000, activate: [], complete: ['fairness'], edges: { active: [], complete: ['e-planner-fairness'] }, message: { from: 'Fairness Agent', content: 'Bias score: 0.03 (Acceptable) — No demographic skew', type: 'success' } },
      { id: 'approval', label: 'Approval', delay: 2000, activate: ['approval'], complete: [], edges: { active: ['e-risk-approval', 'e-matching-approval', 'e-fairness-approval'], complete: [] }, message: { from: 'Approval Agent', content: 'Recommendation generated — Johnson Family (Match: 30%)', type: 'info' } },
      { id: 'approval-done', label: 'Approved', delay: 5000, activate: [], complete: ['approval'], edges: { active: [], complete: ['e-risk-approval', 'e-matching-approval', 'e-fairness-approval'] }, message: { from: 'Approval Agent', content: 'Supervisor approval granted — All criteria validated', type: 'success' } },
      { id: 'monitor', label: 'Monitoring', delay: 3000, activate: ['monitoring'], complete: [], edges: { active: ['e-approval-monitoring'], complete: [] }, message: { from: 'Monitoring Agent', content: 'Placement record created — Johnson Family assigned', type: 'success' } },
      { id: 'monitor-done', label: 'Active', delay: 4000, activate: [], complete: ['monitoring'], edges: { active: [], complete: ['e-approval-monitoring'] }, message: { from: 'Monitoring Agent', content: '21-day adjustment tracking initiated', type: 'success' } },
    ]
    ...
    DEMO_EVENTS.forEach((evt) => {
      ...
      const timer = setTimeout(() => {
        setAgentStates((prev) => {
          const next = { ...prev }
          evt.activate.forEach((id) => {
            next[id] = { ...next[id], status: 'active' as AgentStatus, confidence: Math.min(98, (next[id]?.confidence ?? 0) + 40) }
          })
          evt.complete.forEach((id) => {
            next[id] = { ...next[id], status: 'completed' as AgentStatus, confidence: Math.min(99, (next[id]?.confidence ?? 0) + 30) }
          })
          return next
        })
        ...
        if (evt.message) {
          setMessages((prev) => [...prev, { id: generateId(), timestamp: new Date(), ...evt.message! }])
        }

        setSteps((prev) =>
          prev.map((s) => {
            if (evt.activate.includes(s.agentId as AgentType) && s.status === 'pending') {
              const agentName = s.agentId.charAt(0).toUpperCase() + s.agentId.slice(1) + ' Agent'
              return { ...s, status: 'active' as const, agentName }
            }
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          {isRunning && (
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: 'auto' }}
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-surface-alt border border-border"
            >
      ...
      {/* ── Main: Graph (80%) + Sidebar (20%) ─────────────────── */}
      <div className="flex-1 flex gap-3 min-h-0">
        {/* Graph – 80% */}
        <GlassCard className="relative flex-[4] min-w-0 overflow-hidden p-2 rounded-2xl">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(56,189,248,0.06),transparent_30%),radial-gradient(circle_at_bottom,rgba(16,185,129,0.04),transparent_30%)]" />
        ...
        <GlassCard className="flex-[1] min-w-[280px] flex flex-col p-0 overflow-hidden rounded-2xl">
          ...
          {/* Agent mini-stats in sidebar footer */}
      ...
      <div className="grid grid-cols-3 gap-3 shrink-0">
        <GlassCard className="p-2.5 rounded-xl">
          ...
          <motion.p
            ...
            className="text-lg font-bold text-foreground font-mono mt-1"
          >
        ...
        <GlassCard className="p-2.5 rounded-xl">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] text-muted-foreground">Polling</span>
          </div>
          <p className="text-lg font-bold text-foreground font-mono mt-1">8s</p>
        </GlassCard>
      </div>
    </motion.div>
  )
